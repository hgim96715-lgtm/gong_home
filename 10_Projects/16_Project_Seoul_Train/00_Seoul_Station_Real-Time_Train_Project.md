...
>코레일톡으로 확인해 가며 구상하고 만들고 있다.
>이 프로젝트의 단점은 1. KXT-산천, KTX-청룡에 대한 것은 배제가 되어있다.(단 KTX 열차 시간은 정확함)
>2. 이 기차가 어디를 거치는 것 까지는 데이터를 가질수 없어서 도착역만나와서 Superset에 어디로 많이갈까?에 대한 통계가 부족할수도 있다.,
>3. 또한 KTX-산천, 청룡은 배제되기 때문에 노선별 도착 평균 지연 되는 곳이 두개가 추가되면 달라질수도 있지 않을까 생각한다.
