...
`@st.cache_data`를 쓰면 한 번 불러온 데이터를 영원히 기억합니다. 
하지만 주식 데이터나 GitHub API처럼 **주기적으로 바뀌는 데이터**라면? 영원히 기억하면 옛날 데이터만 보게 됩니다!

이때 `ttl` (Time To Live, 생존 시간) 파라미터를 초(seconds) 단위로 설정합니다.

- **`ttl=300`**: 300초(5분) 동안만 기억하고, 5분이 지나면 캐시를 버리고 API를 다시 호출합니다.
- **실무 활용:** GitHub API는 1시간에 60번 호출 제한이 있으므로 `ttl=3600`(1시간)으로 설정하여 차단을 완벽하게 방어합니다.

```python
# 5분(300초)마다 최신 마크다운 글을 다시 긁어옵니다!
