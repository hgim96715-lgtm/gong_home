...
[공식 설치 가이드](https://airflow.apache.org/docs/apache-airflow/stable/howto/docker-compose/index.html)
...
### (중요) docker-compose.yaml 수정

**Airflow가 Docker 컨테이너를 직접 실행/제어(DockerOperator 사용)하려면** 아래 설정을 반드시 추가해야 합니다.
다운로드 받은 `docker-compose.yaml` 파일을 열고, `airflow-worker` 서비스 부분(또는 공통 설정 `x-airflow-common`의 volumes)을 찾아 아래 내용을 추가해 주세요.
