...
* **"내 컴퓨터 환경 말고, 깨끗한 컨테이너에서 실행하고 싶어?"** -> **`DockerOperator`를 가져다 씀**
* 이렇게 **미리 정의된 클래스(Pre-defined components)** 를 조립만 하면 복잡한 기능을 쉽게 구현할 수 있습니다.

---
## 실무 맥락에서의 사용 이유

실무 DAG의 90%는 `PythonOperator`, `BashOperator`, `KubernetesPodOperator`(또는 `DockerOperator`) 3가지로 이루어집니다.

특히 데이터 엔지니어는 다음을 결정해야 합니다.
- **"내가 직접 파이썬으로 ETL을 짤 것인가"** (`PythonOperator`)
- **"이미 짜여진 dbt나 Spark를 실행만 시킬 것인가"** (`BashOperator`)
- **"라이브러리 버전 충돌 없이 격리된 환경에서 돌릴 것인가"** (**`DockerOperator`**)
	- 예: Task A는 Pandas 1.0이 필요하고, Task B는 Pandas 2.0이 필요할 때 사용
...
1.  **Action Operator (행동대장):**
    * 실제 연산이나 명령을 수행합니다.
    **예: `BashOperator`, `PythonOperator`, **`DockerOperator` (컨테이너 실행)**
...
---
### DockerOperator (환경 격리 전문가)

`PythonOperator`만 쓰다 보면 **"Dependency Hell(의존성 지옥)"** 에 빠지기 쉽습니다.
(하나의 Airflow 환경에 너무 많은 라이브러리가 깔려서 꼬이는 현상)

**역할:** Airflow Worker가 직접 일을 하지 않고, **"도커에게 이미지를 주고 컨테이너를 띄워서 실행시킨 뒤 결과만 받습니다."**

**장점:**

- 태스크마다 다른 파이썬 버전/라이브러리 사용 가능.
- 작업이 끝나면 컨테이너가 사라지므로 뒷정리가 깔끔함.
- **Spark**나 **DBT** 같은 무거운 작업을 돌릴 때 매우 유용함.
...
# DockerOperator 정의 (격리된 컨테이너 실행)
task3 = DockerOperator( 
	task_id='run_in_container',
	 image='python:3.9-slim', # 4-1. 사용할 도커 이미지 
	 command='python -c "print(\'I am running in a container!\')"', # 4-2. 실행할 명령어 
	 network_mode='bridge', # 4-3. 네트워크 설정 
	 auto_remove=True, # 4-4. 끝나면 컨테이너 삭제 
	 dag=dag 
 )
