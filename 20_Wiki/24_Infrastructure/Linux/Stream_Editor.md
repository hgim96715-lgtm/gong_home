...
### E. 특정 줄만 쏙 뽑아보기 (Printing) 

`sed`를 파일 내용을 수정하는 데만 쓰지 않고, **원하는 구간만 읽는 용도**로 쓸 때 사용하는 핵심 문법입니다. 
(`head`와 `tail`을 섞어 쓰는 것보다 훨씬 빠르고 타이핑도 짧습니다.)

**문법:** `{bash}sed -n '시작줄,끝줄p' 파일명`

```bash
# 1. 특정 범위 출력 (Range)
# "12번째 줄부터 22번째 줄까지만 보여줘"
...
- `-n` (No-printing / 은신 모드):
	- `sed`는 기본적으로 "읽은 모든 줄을 화면에 출력"하려는 성질이 있습니다.
	- `-n`을 붙여서 **"일단 아무것도 출력하지 말고 조용히 있어!"** 라고 입을 막습니다
- `p` (Print / 말하기):
	- `'12,22p'`는 "12~22번째 줄을 만났을 때만 **출력(print)** 해라"라는 명령입
	- 결국 **"다 조용히 하고(n), 내가 시킨 구간만 말해(p)"** 가 되는 것이죠.

### F. 상대 좌표로 뽑기 (+ 기호)

"12번째 줄부터 **10개**를 가져와"라고 할 때, 끝 번호(22)를 계산하기 귀찮다면 `+`를 씁니다.

```bash
# 12번째 줄부터 시작해서, 뒤로 10줄 더(Total 11줄) 보여줘
# head -n 11과 비슷한 느낌
