...
#  변수 =~ 패턴 
if [[ "$country" =~ [aA] ]]; then
    echo "$country 에 a 또는 A 가 포함됨"
fi

# ! 로 부정 (포함되지 않으면)
if [[ ! "$country" =~ [aA] ]]; then
    echo "$country 에 a/A 없음"
fi
```

```bash
# 실전 예시 — a 또는 A 가 없는 나라만 출력
...
[aA] 패턴 읽는 법:
  [ ]  = 문자 클래스 (괄호 안 문자 중 하나)
  aA   = a 또는 A
  [aA] = a 나 A 가 하나라도 있으면 매칭

  Korea    → e에 a 포함 → 매칭 ✅
  Japan    → a 포함     → 매칭 ✅
  UK       → a/A 없음   → 매칭 ❌  → ! 조건에서 출력
```
...
```
[ ] vs [[ ]] 차이:
  [ ]   단일 괄호  POSIX 표준 / 정규식 미지원
  [[ ]] 이중 괄호  Bash 전용  / =~ 정규식 지원 / && || 사용 가능
...
| <code>\|</code> | OR (이중 괄호)  | `[[ "$x" -eq 1 \|"$x" -eq 10 ]]`   |
