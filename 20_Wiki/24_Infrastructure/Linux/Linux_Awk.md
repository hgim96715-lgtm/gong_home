...
## 문자열 안에 $1 넣기 — 자주 하는 실수 ⭐️

```bash
# ❌ 큰따옴표 안에 $1 쓰면 그냥 문자열로 출력됨
awk '{ print "이름: $1" }' data.txt
# 이름: $1   ← $1 이 치환 안 됨!

# ❌ printf 도 마찬가지
awk '{ printf "Not all scores are available for $1\n" }' data.txt
# Not all scores are available for $1  ← 그대로 출력
```

```
이유:
  awk 에서 " " 큰따옴표 안은 그냥 문자열
  $1 을 변수로 인식 안 함
  → %s 로 자리 표시 후 뒤에 변수를 인자로 전달해야 함
```
