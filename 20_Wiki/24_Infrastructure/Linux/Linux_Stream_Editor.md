...
---
## A-1. `&` — 매칭된 문자열 그대로 재사용 ⭐️

```
&  = 패턴에 매칭된 문자열 전체를 그대로 가져옴

흔한 실수:
  sed 's/thy/{thy}/g'   ❌ "thy" 라는 글자를 찾아서 {thy} 로 고정 교체
  sed 's/thy/{&}/g'     ✅ "thy" 를 찾아서 {매칭된값} 으로 감싸기
                            → thy 뿐 아니라 대소문자 변형에도 모두 적용됨
```


```bash
# 매칭된 단어를 {} 로 감싸기
echo "thou art thy servant" | sed 's/thy/{&}/gI'
# thou art {thy} servant

# 대소문자 무시(I) + & 조합 → 원본 대소문자 유지하면서 감싸기
echo "Thy will be done. thy kingdom" | sed 's/thy/{&}/gI'
# {Thy} will be done. {thy} kingdom
# ↑ 원본 대소문자 그대로 유지

# 숫자를 [] 로 감싸기
echo "score: 95 point: 87" | sed 's/[0-9][0-9]*/[&]/g'
# score: [95] point: [87]
...
& 활용 패턴:
  감싸기     's/패턴/{&}/g'   → {매칭값}
  따옴표     's/패턴/"&"/g'   → "매칭값"
  접두어 추가  's/패턴/PREFIX_&/g'  → PREFIX_매칭값
  접미어 추가  's/패턴/&_SUFFIX/g'  → 매칭값_SUFFIX
```
...
```bash
# 이름:점수 → 점수:이름 으로 순서 바꾸기
echo "Alice:95" | sed 's/\([A-Za-z]*\):\([0-9]*\)/\2:\1/'
# 95:Alice
# \1 = Alice  \2 = 95

# 날짜 포맷 변환 YYYY-MM-DD → DD/MM/YYYY
echo "2026-03-14" | sed 's/\([0-9]*\)-\([0-9]*\)-\([0-9]*\)/\3\/\2\/\1/'
# 14/03/2026

# 전화번호 앞 3자리만 추출
echo "010-1234-5678" | sed 's/\([0-9]*\)-.*/\1/'
# 010

# 중복 단어 제거 (앞 단어 = 뒤 단어인 경우)
echo "the the cat sat" | sed 's/\([a-z]*\) \1/\1/'
# the cat sat
```

```
BRE (기본 정규식) 에서는 \( \) 로 그룹 묶음
ERE (-E 옵션) 에서는 ( ) 로 그룹 묶음 → 역슬래시 불필요
```

```bash
# ERE (-E) 로 더 간결하게
echo "Alice:95" | sed -E 's/([A-Za-z]+):([0-9]+)/\2:\1/'
# 95:Alice
```
...
```
매칭된 전체를 감싸거나 앞뒤에 뭔가 붙일 때  →  &
매칭된 일부만 추출하거나 순서를 바꿀 때     →  \1 \2
```
...
```text
a = append (after)  → 매칭된 줄 바로 아래에 추가
i = insert (before) → 매칭된 줄 바로 위에 추가

암기:
  a = After  = 아래
  i = Insert = 위 (끼워 넣으면 원래 줄이 밀려 내려감)
```
...
|`'s/thy/{thy}/g'` 가 안 바뀜|`thy` 고정 문자열로 교체 → 대소문자 변형 못 잡음|`'s/thy/{&}/gI'` — `&` 로 원본 유지|
|`\1` 캡처 그룹이 안 됨|BRE 에서 괄호를 `( )` 로 씀|`\( \)` 사용 또는 `-E` 옵션으로 ERE 전환|
