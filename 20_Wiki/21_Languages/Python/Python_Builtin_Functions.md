...
### ③ 자주 하는 실수: `for`문 안에 넣지 마세요

`min`이나 `max` 함수는 그 자체가 **"데이터를 처음부터 끝까지 훑어보는 기능"** 을 가지고 있습니다. 
굳이 `for`문을 돌려서 하나씩 꺼내 줄 필요가 없습니다.
...
# 실수: for문으로 굳이 값을 하나 꺼내서(c) min에 넣음
for c in arr:
    print(min(c)) 
    # 💥 TypeError 발생! 
    # 이유: 숫자 하나(int)는 쪼갤 수 없어서 최소값을 못 구함.
```

[✅ 좋은 예: 리스트 통째로 던지기]

```python
# 정답: 배열(리스트)을 그대로 넣으세요.
print(min(arr)) # 1
```

**💡 한 줄 요약:** `max()`와 `min()`은 **"그릇(List, Iterable)"** 을 통째로 받아야 작동합니다. 낱개(내용물)를 주면 에러가 납니다!
