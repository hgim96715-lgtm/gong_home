...
- 거창하게 `def`로 정의해서 이름을 붙이기엔 너무 간단하고, 딱 한 번만 쓰고 버릴 함수가 필요할 때 사용합니다.

- 문법 : `{python}lambda <입력변수>: <반환값>`

## 핵심 실수: "정의" vs "실행" 헷갈리지 말기

내가 겪었던  **실수**. 람다는 정의만 한다고 실행되지 않습니다.

```python
### ❌ 틀린 예시 (함수 덩어리만 있음)
# 람다 함수 자체를 리스트에 넣은 것 (계산 안 됨!)
answer = [lambda x: sum(x), [1, 2, 3]]
# 결과: [<function <lambda>...>, [1, 2, 3]]
```

### 맞는 예시 1: 괄호로 즉시 호출 

람다 전체를 괄호 `()`로 감싸고, 바로 뒤에 인자를 붙여서 **"지금 당장 실행해!"** 라고 명령합니다.

```python
num_list = [1, 2, 3]

# (함수)(재료) -> 이렇게 괄호를 두 번 써야 실행됩니다.
# 1. sum([1,2,3]) = 6
# 2. 6 ** 2 = 36
result = (lambda x: sum(x)**2)(num_list)
...
람다는 혼자 쓰기보다 `map`이나 `filter`라는 **운반책(Vehicle)** 에 태워 보낼 때 진가를 발휘합니다.

```python
# map이 데이터를 하나씩 꺼내서 람다에게 먹여줌
answer = list(map(lambda x: x + 10, [1, 2, 3]))
# 결과: [11, 12, 13]
```

---
## 문법 해부 (Syntax Anatomy) 
