...
**나의 오답 노트 (단골 착각 주의!):**
`for k, v in 딕셔너리.items():` 코드를 볼 때, 순간적으로 리스트를 다루는 것처럼 **`k`가 '인덱스(순서 번호 0, 1, 2...)'라고 착각하기 아주 쉽습니다!** (마치 `enumerate()`를 쓸 때처럼요.) 하지만 딕셔너리에는 순서 번호라는 개념 자체가 없습니다. 여기서 `k`는 무조건 **'Key(키 이름)'**, `v`는 **'Value(값)'** 이라는 사실을 절대 잊지 마세요
