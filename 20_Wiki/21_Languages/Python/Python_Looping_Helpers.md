...
---
## 루프 흐름 제어: `continue`의 함정

반복문 안에서 `continue`를 만나면 **그 즉시 아래 코드를 무시하고 다음 바퀴로 넘어갑니다.**
초보자들이 가장 많이 하는 실수 중 하나가 바로 이 **"결론 없는 `continue` 뒤처리"** 를 빼먹는 것입니다.

### 핵심 요약 (시험/실무 단골 포인트)

- `continue`는 **"결론을 내리지 않고 그냥 패스"** 하는 역할입니다.
- 조건에 맞아 `return` 이나 `break`로 끝나지 않고, **모든 반복이 `continue`로만 끝났을 때(루프가 다 돌았을 때) 최종적으로 어떤 결론을 내릴지 반드시 루프 바깥에 적어줘야 합니다.**

#### [실전 예시] 날짜 비교하기 로직 (`zip` + `continue`)

두 배열 `[연, 월, 일]`을 앞에서부터 차례로 비교하다가 크기가 다르면 바로 승패를 결정하고, **숫자가 같으면 다음 단계(월 -> 일)로 넘어가는(`continue`)** 코드입니다.

```python
def solution(date1, date2):
    for a, b in zip(date1, date2):
        if a < b:
            return 1   # date1이 더 빠르다! (결론 탕탕)
        elif a > b:
            return 0   # date1이 더 느리다! (결론 탕탕)
        else:          # a == b (숫자가 똑같다면?)
            continue   # "아직 승부를 못 냈으니 다음 순서(월/일) 비교해봐!" 하고 패스

    # ⭐⭐ 이 줄이 없으면 대형 버그 발생! ⭐⭐
    # 만약 date1과 date2가 완벽히 똑같은 날짜라면?
    # 연, 월, 일 모두 else에 걸려서 continue만 3번 하다가 루프가 끝나버립니다.
    # 함수가 아무것도 반환하지 않고 None을 뱉게 되므로,
    # "끝까지 다 돌아봤는데도 승부가 안 났다면(완전히 같으면) 0을 줘라!" 라고
    # 루프가 끝난 자리에 반드시 최종 결론을 적어주어야 합니다.
    return 0
```
