...
**Q. "GROUP BY 절 없이 HAVING 절만 단독으로 쓸 수 있을까?"**

- **정답은 "가능하다(에러 안 남)"이다.**
- 문법적으로 `GROUP BY`가 없어도 `HAVING`을 쓰면, **테이블 전체를 하나의 거대한 그룹으로 간주**하여 집계 함수를 적용한다. (단, 일반 컬럼은 SELECT 절에 올 수 없고 집계 함수만 올 수 있음). 일반적인 실무 패턴은 아니지만 시험에서 틀린 보기로 낚기 위해 자주 출제된다.