...
- **1장. 데이터 모델링의 이해**
    ...
    - [[ERD_Components]] : 엔터티(Entity), 속성(Attribute), 관계(Relationship) (`카디널리티`, `식별/비식별`)
    - [[SQL_Keys_and_Identifiers]] : PK와 FK의 관계 (`본질식별자`, `인조식별자`, `주식별자`, `복합식별자`)

- **2장. 데이터 모델과 성능**
    
    - [[Normalization_Theory]] : 테이블을 쪼개는 기술, 정규화 (`1NF`, `2NF`, `3NF`, `함수적 종속성`)
    - [[De_Normalization]] : 성능을 위해 다시 합치는 기술, 반정규화 (`테이블 병합`, `컬럼 중복`, `파티셔닝`)
