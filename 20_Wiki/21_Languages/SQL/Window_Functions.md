...
5. **누적 합계(Running Total):** "1월 1일부터 오늘까지 **누적**으로 얼마 벌었나?" (`SUM` + `OVER(ORDER BY)`) 

---
## Code Core Points: 3가지 핵심 패턴

집계 함수(`SUM`, `MAX`, `AVG`, `COUNT`) 뒤에 `OVER`가 붙으면 작동 방식이 달라집니다.

- **`OVER()` (빈 괄호):** **"전체 통틀어서"** (테이블 전체 기준 집계)
- **`OVER(PARTITION BY A):`** **"A 그룹끼리만 뭉쳐서"** (그룹별 집계)
- **`OVER(ORDER BY B):`** **"순서대로 쌓아가면서"** (누적 집계) ⭐️
...
---
## 상세 분석: 비율 구하기 & 누적 합계 (PostgreSQL)

**상황:** 직원들의 급여(`salary`) 정보를 보면서, 전체 평균과 비교하고 싶고, 입사일 순서대로 급여가 어떻게 쌓이는지(누적) 보고 싶다.
...
SELECT
    ...
    -- 1. [전체 기준] 회사 전체에서 제일 많이 받는 금액은?
    -- GROUP BY 없이도 전체 MAX값을 모든 행에 붙여줌.
    MAX(salary) OVER() AS "전사_최고연봉",

    -- 2. [비율 계산] 내가 받는 돈은 전사 최고 연봉의 몇 %인가?
    -- (내 연봉 / 최고 연봉) 계산이 한 줄로 됨!
    salary / MAX(salary) OVER() AS "최고연봉_대비_비율",

    -- 3. [그룹 기준] 우리 부서 사람들끼리만 따졌을 때 평균은?
    AVG(salary) OVER(PARTITION BY department) AS "부서_평균연봉",

    -- 4. [누적 합계] 입사일(hire_date) 순서대로 월급을 계속 더해간다면? (Running Total)
    --  OVER 안에 'ORDER BY'가 들어가면 '누적'으로 바뀝니다! 
    SUM(salary) OVER(ORDER BY hire_date) AS "누적_급여_합계"
...
|**name**|**salary**|**전사_최고연봉**|**부서_평균연봉**|**누적_급여_합계**|**해석**|
|---|---|---|---|---|---|
|김철수|300|**500**|350|**300**|첫 번째 사람 (300)|
|이영희|400|**500**|350|**700**|300 + 400 = 700 (누적)|
|박민수|500|**500**|500|**1200**|700 + 500 = 1200 (누적)|
...
###  ⑤ "`SUM(x) OVER()`랑 `SUM(x) OVER(ORDER BY y)`는 같은 거 아닌가요?"

- **완전히 다릅니다! (천지차이)**
- `OVER()` (빈 괄호): 그냥 **전체 합계** (100+200+300 = 600)이 모든 줄에 똑같이 나옵니다.
- `OVER(ORDER BY ...)`: **누적 합계** (100, 300, 600...)로 줄마다 다르게 나옵니다.
- **Tip:** 순서(`ORDER BY`)를 정해주는 순간 SQL은 "아, 차례대로 계산하라는 거구나?"라고 인식해서 **누적(Running)** 계산을 시작합니다