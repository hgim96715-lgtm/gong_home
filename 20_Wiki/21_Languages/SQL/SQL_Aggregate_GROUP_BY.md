...
```
┌──────────────────────────────────────────────────────────────┐
│  GROUP BY = 데이터를 기준으로 묶어서 집계                      │
│  SELECT 의 기준 컬럼 → GROUP BY 에 반드시 포함                 │
│                                                              │
│  COUNT 비교:                                                  │
│  COUNT(*)   = COUNT(1) = 행 전체 수 (NULL 포함)               │
│  COUNT(컬럼) = NULL 제외한 값 있는 행 수                       │
│  COUNT(DISTINCT 컬럼) = 중복+NULL 제거한 고유 값 수            │
│                                                              │
│  COUNT(1) = COUNT(*): 성능 차이 없음 (옵티마이저가 동일 처리)  │
│                                                              │
│  AVG 함정: NULL 행을 분모에서 제외 → [100,200,NULL] = 150     │
│  NULL 포함 평균 원하면: AVG(COALESCE(컬럼, 0))                │
│                                                              │
│  WHERE  = 그룹핑 전 필터 (집계함수 ❌)                         │
│  HAVING = 그룹핑 후 필터 (집계함수 ✅)                         │
│  NULL 제외는 WHERE 먼저 → 성능상 유리                          │
│                                                              │
│  중첩 집계 AVG(SUM()) → 에러! → 서브쿼리로 단계 분리           │
│  HAVING 단독 사용 → 문법상 가능 (전체를 하나의 그룹으로 처리)  │
│                                                              │
│  순위 번호 필요 → RANK                                        │
│  횟수·건수 필요 → COUNT(*) + GROUP BY + HAVING               │
└──────────────────────────────────────────────────────────────┘
```
...
- [[SQL_Group_Functions]] — ROLLUP · CUBE · GROUPING SETS (다차원 집계)
- [[SQL_Window_Functions]] — RANK · DENSE_RANK · ROW_NUMBER
- [[SQL_NULL_Functions]] — COALESCE · NVL · AVG 와 NULL
- [[SQL_SubQuery]] — 중첩 집계 시 서브쿼리 활용
- [[HAVING_vs_WHERE]] — WHERE · HAVING 상세 비교