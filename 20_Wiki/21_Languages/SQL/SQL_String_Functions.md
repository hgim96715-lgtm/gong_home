...
- **문법:** `LENGTH(문자열)`
- **설명:** 글자가 총 몇 개인지 숫자로 반환합니다.

**DBMS 차이점:**
- **Oracle:** `LENGTH('SQLD')` ➡️ `4`
- **SQL Server:** `LEN('SQLD')` ➡️ `4` (`LEN`을 씁니다!)
- **PostgreSQL:** 오라클과 똑같이 **`LENGTH('SQLD')`** ➡️ `4` 를 씁니다. (참고로 ANSI 표준 문법인 `CHAR_LENGTH()`를 써도 똑같이 작동합니다.)
...
- **실무 예시:** `REPLACE('010-1234-5678', '-', '')` ➡️ `'01012345678'` (하이픈 제거)
- **SQLD 꿀팁 (제거 스킬):** 세 번째 인자인 '바꿀 문자'를 명시하지 않고 생략해 버리면, 해당 문자를 다른 걸로 바꾸는 게 아니라 **문자열에서 완전히 삭제(제거)** 해 버립니다!
...
-- 2) 💥 바꿀 문자를 생략하여 아예 삭제하기 (Oracle 전용)
-- 결과: '01012345678' (하이픈을 흔적도 없이 지워버림)
SELECT REPLACE('010-1234-5678', '-') FROM DUAL; 

-- ❌ [SQL Server / PostgreSQL] 에러 발생!
-- 이 DB들은 인자가 무조건 3개 있어야 합니다. 생략하면 문법 에러가 납니다.
-- 따라서 지우고 싶을 때는 반드시 빈 문자열('')을 명시해줘야 합니다.
SELECT REPLACE('010-1234-5678', '-', '') FROM DUAL;
```
