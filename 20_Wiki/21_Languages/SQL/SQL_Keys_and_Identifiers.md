---
aliases:
  - 식별자 완벽 정리
  - DB Identifier Theory
  - 주식별자와 보조식별자
  - 식별자 분류
  - 인조식별자vs본질식별자
tags:
  - SQL
related:
  - "[[Data_Modeling_Overview]]"
  - "[[SQL_ERD_Components]]"
  - "[[00_SQL_HomePage]]"
  - "[[SQL_DML_CRUD]]"
---
# SQL_Keys_and_Identifiers : 키와 식별자의 모든 것

> **핵심 요약** **"식별자(Identifier)"** 란 엔터티(Entity) 내에서 인스턴스들을 구분할 수 있는 **구분자**이다. 
> 단순히 PK만 있는 것이 아니라, **대표성 · 생성 원인 · 속성 수 · 대체 여부**에 따라 다양하게 분류된다.

---
---
## 키(Key) 종류

>**"키(Key)"** 란 테이블에서 행(Row)을 유일하게 식별하거나, 테이블 간의 관계를 정의하기 위해 사용하는 속성(또는 속성의 집합)이다.

### 키 종류 한눈에 보기

```text
슈퍼키 (Super Key)
  └── 후보키 (Candidate Key)  ← 유일성 + 최소성 만족
        ├── 기본키 (Primary Key)  ← 후보키 중 대표로 선택된 것
        └── 대체키 (Alternate Key)  ← 후보키 중 기본키가 되지 못한 나머지
외래키 (Foreign Key)  ← 다른 테이블의 PK를 참조
```

----
### ① 슈퍼키 (Super Key)

> 유일성은 만족하지만 최소성은 만족하지 않아도 되는 키. 쉽게 말해 **"행을 구분할 수 있는 속성의 모든 조합"**.

```text
사원 테이블: 사번, 이름, 주민번호, 부서

슈퍼키가 될 수 있는 조합:
✅ {사번}
✅ {주민번호}
✅ {사번, 이름}       ← 유일하지만 이름은 불필요 → 최소성 위반
✅ {사번, 이름, 부서} ← 유일하지만 이름, 부서 불필요 → 최소성 위반
```

---
### ② 후보키 (Candidate Key)

> **유일성 + 최소성** 을 동시에 만족하는 키. 기본키가 될 자격이 있는 후보들.

```text
사원 테이블에서 후보키:
✅ {사번}      → 유일 + 최소 → 후보키 ⭕
✅ {주민번호}  → 유일 + 최소 → 후보키 ⭕
❌ {사번, 이름} → 유일하지만 최소성 위반 → 후보키 ❌
```

### ③ 기본키 (Primary Key, PK)

> 후보키 중에서 **대표**로 선택된 키. `NOT NULL + UNIQUE` 가 자동으로 보장된다.

```text
{사번}, {주민번호} 둘 다 후보키인데
→ 보안상 주민번호는 노출 위험
→ {사번} 을 기본키로 선택

사번    이름    주민번호    부서
──────────────────────────────
001    홍길동   9001011... 개발팀  ← 사번이 PK
```

#### 기본키 특징:

- NULL 불가
- 중복 불가
- 테이블당 1개만 지정 가능
- 변경되지 않아야 함 (불변성)

---
### ④ 대체키 (Alternate Key)

> 후보키 중에서 **기본키로 선택되지 못한** 나머지 키.  
>  물리 테이블에서 **Unique Index** 로 구현된다.

```text
후보키: {사번}, {주민번호}
기본키: {사번} 선택
대체키: {주민번호} ← 기본키는 못 됐지만 여전히 유일성 보장

→ DB 에서는 주민번호 컬럼에 UNIQUE 제약조건으로 구현
```

---
### ⑤ 외래키 (Foreign Key, FK)

> **다른 테이블의 기본키를 참조**하는 키. 테이블 간의 관계를 정의하고 참조 무결성을 보장한다.

```text
사원 테이블          부서 테이블
──────────────      ──────────────
사번 (PK)           부서코드 (PK)
이름                부서명
부서코드 (FK) ──────▷

사원.부서코드 → 부서.부서코드 를 참조
부서 테이블에 없는 부서코드는 사원에 입력 불가 (참조 무결성)
```

#### 외래키 특징:
- NULL 허용 (부서 미배정 사원 가능)
- 중복 허용 (같은 부서에 여러 사원 가능)
- 참조하는 테이블의 PK 값만 입력 가능
- 부모 행 삭제 시 자식 행 처리 필요 (CASCADE, SET NULL 등)

---
## 주식별자의 4대 특징 ⭐

주식별자(Primary Identifier)가 되려면 아래 4가지 조건을 **반드시** 만족해야 합니다.

> 💡 **암기 팁: "유.최.불.존"**

| 특징                     | 설명                          | 위반 예시                       |
| :--------------------- | :-------------------------- | :-------------------------- |
| **유**일성 (Uniqueness)   | 모든 인스턴스를 유일하게 구분해야 함        | 이름 → 동명이인이 있으면 탈락           |
| **최**소성 (Minimality)   | 유일성을 만족하는 **최소한의 속성 수**로 구성 | 사번+이름 → 사번만으로 구분되는데 이름은 불필요 |
| **불**변성 (Immutability) | 식별자의 값은 변하지 않아야 함           | 부서명 → 부서 이름이 바뀌면 식별자가 흔들림   |
| **존**재성 (Not Null)     | 값이 반드시 존재해야 함 (NULL 불가)     | 취미 → 없는 사람이 있을 수 있음         |
|                        |                             |                             |

---
##  SQLD 표기법 — 테이블 다이어그램에서 PK 구분하기

SQLD 문제에서 테이블 구조는 아래처럼 표기됩니다.

>💡 **핵심 규칙:** 가로선 **위쪽 = PK**, 가로선 **아래쪽 = 일반 컬럼** PK는 자동으로 `NOT NULL + UNIQUE` 조건을 가집니다.

### 케이스 1 — 단일 PK (가장 흔한 형태)


```text
┌─────────────────┐
│      COL1       │  ← PK 1개
├─────────────────┤
│ COL2            │  ← 일반 컬럼
│ COL3 DEFAULT 'D'│
└─────────────────┘
```

### 케이스 2 — 복합 PK ⭐ (시험 자주 출제)

```text
┌─────────────────┐
│  회원번호(FK)   │  ← PK 2개가 선 위쪽에 함께 존재
│  배송지순번     │  ← = 복합식별자(Composite Identifier)!
├─────────────────┤
│  배송지         │  ← 일반 컬럼
└─────────────────┘
```

>선 위쪽에 **컬럼이 2개 이상** 있으면 → **복합 PK** 이 경우 두 컬럼이 **합쳐져야** 하나의 행을 유일하게 식별할 수 있습니다.

### 실전 예시 (1NF 정규화 문제):

```test
[정규화 전 - 1NF 위반]            [정규화 후 - 1NF 만족 ✅]

┌──────────────┐            회원              배송지
│   회원번호   │         ┌──────────┐    ┌────────────────┐
├──────────────┤         │  회원번호 │    │  회원번호(FK)  │ ← 복합 PK
│   아이디     │  →1NF→  ├──────────┤    │  배송지순번    │ ← 복합 PK
│   이름       │         │  아이디   │    ├────────────────┤
│   배송지1    │ ← 반복!  │  이름     │    │  배송지        │ ← 일반 컬럼
│   배송지2    │ ← 위반   └──────────┘    └────────────────┘
│   배송지3    │
└──────────────┘
```

### 케이스 3 — 일반 컬럼 없이 PK만 존재

```text
┌─────────────────┐
│      COL1       │  ← PK만 있음
├─────────────────┤
│                 │  ← 일반 컬럼 없음 (비어있음)
└─────────────────┘
```

>매핑(교차) 테이블이나 단순 코드 테이블에서 볼 수 있는 형태입니다.

---
## 식별자의 분류 체계 (Classification)

식별자는 바라보는 **관점**에 따라 4가지로 분류됩니다.

```mermaid
mindmap
  root((식별자 분류))
    ①대표성 여부
      주식별자 PK
      보조식별자
    ②생성 위치
      내부식별자
      외부식별자 FK
    ③속성 수
      단일식별자
      복합식별자
    ④대체 여부
      본질식별자
      인조식별자
```

---
## 대표성 여부 — "엔터티를 대표하냐, 아니냐?"

|구분|설명|예시|특징|
|:--|:--|:--|:--|
|**주식별자** (Primary Identifier)|엔터티를 대표하며, FK 참조의 기준이 됨|사원번호, 고객ID|PK로 지정됨|
|**보조식별자** (Alternate Identifier)|유일성은 만족하지만 대표성은 없음|주민등록번호 — 유일하지만 보안상 PK로 안 씀|물리 테이블에서 **Unique Index**로 지정됨|

> 💡 **왜 주민번호를 PK로 안 쓸까?** 유일하긴 하지만, 외부 시스템에 노출될 수 있고 법적으로도 수집이 제한됩니다. 그래서 별도의 `고객ID`를 인조식별자로 만들어 PK로 씁니다.

---
## 생성 위치 — "어디서 왔냐?"

|구분|설명|예시|
|:--|:--|:--|
|**내부식별자** (Internal Identifier)|엔터티 내부에서 스스로 생성된 식별자|부서코드, 주문번호|
|**외부식별자** (Foreign Identifier / FK)|다른 엔터티와의 관계를 통해 외부에서 들어온 식별자|사원 테이블의 '부서코드(FK)'|

---
##  속성의 수 — "몇 개 컬럼으로 구성됐냐?"

|구분|설명|예시|
|:--|:--|:--|
|**단일식별자** (Single Identifier)|하나의 속성으로만 구성됨|고객ID|
|**복합식별자** (Composite Identifier)|두 개 이상의 속성이 묶여서 식별자 역할|수강이력 = `학번` + `과목코드` 가 합쳐져야 유일함|

> ⚠️ **복합식별자 주의사항:** 유일성을 위해 억지로 속성을 추가하지 말 것! **최소성 원칙**에 따라 꼭 필요한 속성만 묶어야 합니다.

---
## 대체 여부 — "업무에 원래 있던 거냐, 만든 거냐?"

|구분|설명|예시|장단점|
|---|---|---|---|
|**본질식별자** (Natural Identifier)|업무에 존재하는 데이터를 그대로 사용|이메일, 주민번호, 사업자번호|✅ 직관적  <br>❌ 값이 길어지거나 변경될 위험 있음|
|**인조식별자** (Surrogate Identifier)|업무적으로는 없지만 편의상 인위적으로 만든 식별자|일련번호(Sequence), 로그ID|✅ 짧고 불변, 관리 편함  <br>❌ 추가 인덱스 필요  <br>❌ 중복 데이터 방지 어려움|

> 💡 **인조식별자를 쓰는 이유:** 복합식별자(주문번호+상품번호)처럼 PK가 두 개 이상인 경우, 이것들을 **하나의 인조 ID로 통합**해서 쓰는 게 관리가 훨씬 편하기 때문입니다. **대리식별자(Surrogate Identifier)** 라고도 합니다.

---
---
##  식별 관계 vs 비식별 관계

부모 테이블의 PK를 자식 테이블이 **"어떻게 물려받느냐"** 에 따라 두 가지로 나뉩니다.

| 구분           | **식별 관계 (Identifying)**                                                                  | **비식별 관계 (Non-Identifying)**                                                          |
| ------------ | ---------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| **ERD 표기**   | **실선** ────                                                                              | **점선** - - - -                                                                        |
| **PK 포함 여부** | 부모의 PK가 자식의 **PK + FK**가 됨                                                               | 부모의 PK가 자식의 **일반 속성(FK)** 으로만 옴                                                       |
| **의존성**      | **강한 의존** "부모 없이는 자식도 없다"                                                                | **약한 의존** "부모 없어도 자식 혼자 식별 가능"                                                        |
| **비유**       | **유전자** — 부모의 피가 나의 정체성                                                                  | **헬스장 회원권** — 내가 누구든 상관없고, 회원권만 가짐                                                    |
| **언제 쓰나**    | 부모 없이 자식이 존재 불가한 경우                                                                      | 대부분의 일반적인 관계                                                                          |
| **연결 고려사항**  | · SQL 조인 횟수를 최소화할 수 있음  <br>· 자식 테이블만 조회해도 부모 정보 식별 가능  <br>· 단, 관계가 깊어질수록 PK 컬럼 수 누적 주의 | · 자식이 독립적인 PK를 가져 구조가 단순함  <br>· 부모 변경 시 자식에 미치는 영향이 적음  <br>· 조회 시 부모 테이블과의 JOIN이 필요 |

---
## 식별 관계 (Identifying) — 실선

자식이 부모의 PK를 **자신의 PK 일부**로 사용하는 경우. 부모가 사라지면 자식도 존재 가치를 잃습니다.

**예시:** `주문` → `주문상품` 주문상품은 주문번호 없이는 존재할 수 없습니다. 주문번호가 곧 나의 PK 일부.

```mermaid
erDiagram
    ORDER ||--|{ ORDER_ITEM : "실선 (PK 공유)"

    ORDER {
        int order_id PK
    }
    ORDER_ITEM {
        int order_id PK, FK "내 PK이자 FK"
        int item_id  PK    "내 PK"
    }
```

---
## 비식별 관계 (Non-Identifying) — 점선

자식이 부모의 PK를 **참고용(FK)** 으로만 가지는 경우. 자식은 독립적인 PK를 가지고 있고, 부모는 단지 참조만 할 뿐입니다.

**예시:** `부서` → `사원` 사원은 부서가 없어도(대기발령) 사번이라는 독립적인 PK가 있습니다.

```mermaid
erDiagram
    DEPARTMENT ||..o{ EMPLOYEE : "점선 (일반 FK)"

    DEPARTMENT {
        int dept_id PK
    }
    EMPLOYEE {
        int emp_id  PK "나만의 독립적인 PK"
        int dept_id FK "그냥 일반 컬럼"
    }
```

> 💡 **설계 원칙:** 반드시 종속적이어야 하는 경우(주문-주문상세, 게시글-댓글)에만 **식별 관계(실선)** 를 고민하고, 나머지는 대부분 **비식별 관계(점선)** 으로 설계합니다.

---


