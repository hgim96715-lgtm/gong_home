...
>**PRIOR 의 좌우 위치는 상관없다. 어느 컬럼에 붙었냐만 본다.**
> `PRIOR A = B` 라면 **A → B 방향**으로 탐색한다고 생각하면 된다. `A = PRIOR B` 라면 **A ← B 방향** (B → A 와 같다)

### 풀이 순서 — 문제에서 이렇게 접근한다

```text
① 표를 보고 계층 구조를 직접 그린다
   (NULL 인 행이 루트, 나머지는 자식)

② PRIOR A = B 에서 A → B 화살표를 그린다

③ 내가 그린 계층 구조와 화살표 방향이 맞는지 확인
```
...
③ 확인:
   사원번호=1(회장) → 매니저번호=1 인 부장 찾기 → 부장 매니저번호=2 인 과장 찾기
   회장 → 부장 → 과장 → 사원  ✅ 계층 구조와 일치 → 순방향
```
...
③ 확인:
   사원(매니저번호=3) → 사원번호=3 인 과장 찾기 → 과장 매니저번호=2 → 부장 찾기
   사원 → 과장 → 부장 → 회장  ✅ 위로 거슬러 올라감 → 역방향
```
...
|`CONNECT_BY_ISCYCLE`|루프(순환) 발생한 행이면 **1**, 아니면 **0** 반환 (`NOCYCLE` 과 함께 사용)|
